...
#Switch Interface Description Chek
#This checks if the Interface Description matches data that is coming from LLDP.
#It will output all the interfaces that lldpRemSysName does not match ifAlias

#Usage: lldp_description_match.py -C <community> -H <hostname>
#Options
...
for port in dataPoints['lldpRemSysName']['port']:
    ...
    if dataPoints['ifDescr']['port'][port] == 6:
        continue
    if str(dataPoints['ifAlias']['port'][port]).lower() not in str(dataPoints['lldpRemSysName']['port'][port]).lower():
        print ("%s %s <----> %s" % ( 
                                                  dataPoints['ifDescr']['port'][port], 
                                                  dataPoints['ifAlias']['port'][port], 
